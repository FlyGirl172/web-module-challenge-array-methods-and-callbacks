...
const final = fifaData.filter((item) => {
...
function getWinnersByYear(data, getFinalsCB, getYearsCB, getWinnersCB) {
    const theWinners = getWinnersCB(data, getFinalsCB);
    const years = getYearsCB(data, getFinalsCB);

    return theWinners.map((item, index) => `In ${years[index]}, ${item} won the world cup!`);
}
...
function getAverageGoals(data) {
    const meanHomeGoals = data.reduce(function (acc, item) {
        return acc + item['Home Team Goals'] + item['Away Team Goals'];
    }, 0)
    return (meanHomeGoals / data.length).toFixed(2)
}
